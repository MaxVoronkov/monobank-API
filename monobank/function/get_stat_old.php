...
function statistic_card_day(){
include('../inludes/db.php');
$card_result = array();
$balance_sf = array();
$card_curr = mysqli_query($connection, "SELECT * FROM `mono_user` WHERE `token` = 'uFHDBSP_PE9mr2VbnJ7WpJFcuY9zcvYh_gpB65mwaph0'");
...
$bal_sart_date = mysqli_query($connection, "SELECT * FROM `mono_transaction` WHERE `date` LIKE '$date_search' AND (
`description` = '$desc_search' OR `description` LIKE '$descor_search' OR `description` LIKE '$descor_search1' OR `description` LIKE '$descor_search2') AND `id_card` = '$mono_id_card' ORDER BY  `date` ASC
              LIMIT 1");
$bal_sart = mysqli_fetch_assoc($bal_sart_date);
$bal_uah_start = floatval($bal_sart['ballance'])*$mono_sell_exchange;
array_push( $day_sf[$datedate], $bal_uah_start);


$bal_finish_date = mysqli_query($connection, "SELECT * FROM `mono_transaction` WHERE `date` LIKE '$date_search' AND (
`description` = '$desc_search' OR `description` LIKE '$descor_search' OR `description` LIKE '$descor_search1' OR `description` LIKE '$descor_search2') AND `id_card` = '$mono_id_card' ORDER BY  `date` DESC
              LIMIT 1");
$bal_finish = mysqli_fetch_assoc($bal_finish_date);
$bal_uah_finish = floatval($bal_finish['ballance'])*$mono_sell_exchange;
...
<?php
	...
	foreach ($card_result as $k => $val) {
			foreach ($val as $key1 => $value1) {
				...
				array_push( $datearrstart["$key1"], $balance_sf[$k][$key1][0] );
				array_push( $datearrfinish["$key1"], $balance_sf[$k][$key1][1]);
		}unset($value1);
	}unset($val);
...
<tr>
	<th><?php echo $key?></th>
	<th><?php echo round(array_sum($datearrstart[$key])/100, 2) ?></th>
<?php
	foreach ($value as $key1 => $value1) {
?>
	<th><?php echo $value1/100?></th>
	
<?php
	}
?>
	<th><?php echo array_sum($value)/100;?></th>
	<th><?php echo round(array_sum($datearrfinish[$key])/100, 2) ?></th>
</tr>
